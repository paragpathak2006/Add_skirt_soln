...
Topology::Topology() {
    //read_file_txt("Part.txt");
    read_file_stl("Part.STL");
    generate_topology();
...
void Topology::read_file_txt(string file) {
    ...
    while (fin >> s) {
        ...
        if (s == "vertex") { fin >> v0[i] >> v1[i] >> v2[i];       
#ifdef DEBUG
...
void Topology::read_file_stl(string s) {
    float x, y, z;
    try {
        StlMesh <float, unsigned int> mesh(s);
